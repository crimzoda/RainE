...
bool bShowNewGraphDialog = false;
bool bShowOpenGraphDialog = false;

ImGuiID mainDockSpaceID;
std::vector<std::shared_ptr<GraphWindow>> graphWindowList;
...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow((int)(1280 * main_scale), (int)(800 * main_scale), "RainE Editor", nullptr, nullptr);
    if (window == nullptr)
        return 1;
...
#endif
    {
        ...
        mainDockSpaceID = ImGui::DockSpaceOverViewport(0, ImGui::GetMainViewport());

        ShowMainMenuBar();

        if (bShowNewGraphDialog) {
            ShowNewGraphDialog();
        }
        if (bShowOpenGraphDialog) {
            ShowOpenGraphDialog();
        }
    ...
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("File")) {
            if (ImGui::MenuItem("New Graph")) {
                bShowNewGraphDialog = true;
            }
            if (ImGui::MenuItem("Open Graph")) {
                bShowOpenGraphDialog = true;
            }
...
void ShowNewGraphDialog() {
    ImGui::Begin("Create new Graph");
    static char str0[128] = "";
    ImGui::InputText("*", str0, IM_ARRAYSIZE(str0));

    if (ImGui::Button("Create")) {
        ImGui::SetNextWindowDockID(mainDockSpaceID, ImGuiCond_Always);
        std::shared_ptr<GraphWindow> graphWindow = std::make_shared<GraphWindow>(false, str0);
